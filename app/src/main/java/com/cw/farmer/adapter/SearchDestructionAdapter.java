...
import com.cw.farmer.activity.SearchDestructionFarmerActivity;
import com.cw.farmer.model.PageItemsDestruction;
import com.cw.farmer.model.SearchContractPageItem;

import java.util.ArrayList;

public class SearchDestructionAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public SearchDestructionAdapter(RecyclerView recyclerView, Context context, ArrayList<PageItemsDestruction> pageItemArrayList) {
    ...
    public void setdata(ArrayList<PageItemsDestruction> feedDataArrayList) {
...
//    }
    ...
    private void populateItemRows(ViewHolder viewHolder, int position) {
        final PageItemsDestruction pageItem = pageItemArrayList.get(position);
        String date="";
        for(int elem : pageItem.getCropDate()){
            date=elem+"/"+date;
        }
        String contractdate="";
        for(int elem : pageItem.getContractDate()){
        ...
        viewHolder.tv_idno.setText("Crop Date: "+removeLastChar(date)+"\nContract Date: "+removeLastChar(contractdate));
        viewHolder.tv_noofunits.setText("No of units: "+pageItem.getUnits());
        ...
        viewHolder.lin_item.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                String finalContractdate = contractdate;
                String finalDate = date;
                Intent intent = new Intent(context, CropDestructionActivity.class);
                intent.putExtra("id",pageItem.getId()+"");
                intent.putExtra("name",pageItem.getFamerName());
                intent.putExtra("accountNumber",pageItem.getAccountNumber());
                intent.putExtra("referenceNo",pageItem.getReferenceNo());
                intent.putExtra("crop_date",removeLastChar(finalDate)+"");
                intent.putExtra("units",pageItem.getUnits()+"");
                intent.putExtra("fileId",pageItem.getFileId()+"");
                intent.putExtra("farmerCode",pageItem.getFarmerCode()+"");
                intent.putExtra("farmerId",pageItem.getFarmerId()+"");
                intent.putExtra("cropDateId",pageItem.getCropDateId()+"");
                intent.putExtra("contractDate",removeLastChar(finalContractdate)+"");
                context.startActivity(intent);
            }
        });
